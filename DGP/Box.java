...
public class Box {
	...
	public Box(Field field, int col, int row, Line northLine, Line southLine,
		...
		// save the lines
		this.line[Box.CIMA] = northLine;
		this.line[Box.BAIXO] = southLine;
		this.line[Box.ESQ] = eastLine;
		this.line[Box.DIR] = westLine;
	...
	public static int oppositeSide(int side) {
		if (side == CIMA) return BAIXO;
		if (side == BAIXO) return CIMA;
		if (side == ESQ) return DIR;
		else return ESQ;
	}
