...
public class State implements ActionListener {
	...
	private static final int PLAYER2_ESPERA_STATE = 4;
	// State: the game has ended
	private static final int FIM_STATE = 5;
	...
	public static String stateString(int state) {
		...
		if (state == PLAYER2_ESPERA_STATE) return "PLAYER2_ESPERA_STATE";
		if (state == FIM_STATE) return "FIM_STATE";
...
//		 + nearestLine);
	...
	public String getVencedor() {
		if(this.computerCounter.getCount() > this.userCounter.getCount()) {
	...
	private void enterState(int newState) {
		...
		if (newState == WANDER_STATE) {
			...
			if (!this.field.hasUndrawnLine()) {
				enterState(FIM_STATE);
				System.out.println("FINISH " + this.getVencedor());
			}
		...
		// entering COMPUTER
			...
			// field contains no undrawn line
			if (!this.field.hasUndrawnLine()) enterState(FIM_STATE);
			// field has an undrawn line
				...
				enterState(PLAYER2_ESPERA_STATE);
			}
		}
		
		// entering COMPUTERWAIT
		else if (newState == PLAYER2_ESPERA_STATE) {
			// start timer (no race condition because Swing callbacks come from
			// one thread)
			this.delayTimer.start();
			// enter the state
			this.currentState = PLAYER2_ESPERA_STATE;
		}
		
		// entering ENDGAME
		else this.currentState = FIM_STATE;
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		// COMPUTERWAIT
		if (this.currentState == PLAYER2_ESPERA_STATE) {
